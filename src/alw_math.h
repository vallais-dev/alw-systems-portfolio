...
// Исправлено: std::isnan / std::isinf для совместимости с хост-компиляцией
// =============================================================================
...
inline HOST_DEVICE void alw_sqrt_dd(double hi, double lo, double& out_hi, double& out_lo) {
    ...
    out_lo = diff / (2.0 * s);
    if (std::isnan(out_lo) || std::isinf(out_lo)) out_lo = 0.0;
}
...
inline HOST_DEVICE void alw_sin_cos_dd(double hi, double lo,
    ...
    out_cos_hi = c;
    out_cos_lo = -s * lo;
    if (std::isnan(out_sin_lo) || std::isinf(out_sin_lo)) out_sin_lo = 0.0;
    if (std::isnan(out_cos_lo) || std::isinf(out_cos_lo)) out_cos_lo = 0.0;
}
    ...
    int refine_iter = 2)
{
    ...
    for (int i = 0; i < n * n; ++i) {
        if (std::isnan(A_hi[i]) || std::isinf(A_hi[i]) || std::isnan(A_lo[i]) || std::isinf(A_lo[i]))
            return false;
    }
    for (int i = 0; i < n; ++i) {
        if (std::isnan(b_hi[i]) || std::isinf(b_hi[i]) || std::isnan(b_lo[i]) || std::isinf(b_lo[i]))
            return false;
    }
    ...
    if (std::isnan(cond_est) || std::isinf(cond_est)) cond_est = 1e15;
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        for (int idx = 0; idx < n; ++idx) {
            ...
            if (iter > 0) {
                ...
                if (std::isnan(omega) || std::isinf(omega)) omega = 0.01;
            }
            prev_grad[idx] = grad;
    ...
    for (int i = 0; i < n; ++i) {
        x_hi[i] = x_new[i].hi;
        x_lo[i] = x_new[i].lo;
        if (std::isnan(x_hi[i]) || std::isinf(x_hi[i])) return false;
    }
